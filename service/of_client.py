...
class OFClient:
    ...
    def _http_call(self, method: str, url: str, *,
        ...
        try:
            # Datacenter providers (e.g. IPRoyal DC) sporadically 403 on
            # CONNECT under load; the same IP works again ~400ms later.
            # Retry once against the SAME self.http session (same proxy IP
            # baked in at construction — see __init__). No proxy lookup,
            # no session rebuild, no rotation: OF sees a single sticky IP.
            for attempt in (0, 1):
                try:
                    if method == "GET":
                        r = self.http.get(url, headers=headers, timeout=self.timeout_s)
                    else:
                        r = self.http.request(method, url, json=json_body,
                                              headers=headers, timeout=self.timeout_s)
                    break
                except requests.exceptions.ProxyError as e:
                    if attempt == 0:
                        log_of.info(
                            "proxy_error_retry account=%s proxy=%s(%s) method=%s url=%s err=%s",
    ...
    def get_messages(self, chat_id: str | int, *, limit: int = 10,
                     order: str = "desc", before_id: str | int | None = None) -> dict:
        """GET /api2/v2/chats/{chat_id}/messages — single page of history.
        Pass `before_id` (last message id from previous page) to paginate older."""
    ...
    def list_users(self, user_ids: list[str | int], *, view: str = "m") -> dict:
        ...
        r = self.http.get(url, headers=self._signed_headers(url), timeout=self.timeout_s)
        if not r.ok:
            raise OFAPIError(f"{r.status_code} for {r.url}\n{r.text[:500]}", response=r)
        return r.json()
    ...
    def subscribers(self, *, type: str = "active", limit: int = 10, offset: int = 0,
                    sort: str = "-users.last_activity", filter: str | None = None) -> dict:
    ...
    # mass_messages: no live path found for listing past mass-msg campaigns.
    # OF's web UI surfaces these inside scheduled_messages above (queued ones)
    # and the send is POST /chats/messages with userLists/groups in the body.
    ...
    def chat_media(self, chat_id: str | int, *, limit: int = 20, offset: int = 0) -> dict:
        """GET /api2/v2/chats/{chat_id}/media — only the messages that have media."""
        return self.get_json(f"{API_BASE}/chats/{chat_id}/media",
                             params={"limit": limit, "offset": offset})
    ...
    def send_mass_message(self, text: str, *,
                          ...
                          scheduled_date: str | None = None) -> Any:
    ...
    def create_post(self, text: str, *,
                    ...
                    expire_period: int | None = None) -> Any:
