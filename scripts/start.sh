...
echo
echo "  Local /inbox: http://${HOST}:${NEXT_PORT}/inbox?t=${TOKEN}"
echo "  Local /ui:    http://${HOST}:${PORT}/ui/?t=${TOKEN}"
if [[ "$want_tunnel" == "true" ]] && command -v tailscale >/dev/null 2>&1; then
  ...
  if [[ -n "$TS_HOST" ]]; then
    echo "  Tailscale:    https://${TS_HOST}/inbox?t=${TOKEN}"
    echo "                https://${TS_HOST}/ui/?t=${TOKEN}"
  fi
fi
