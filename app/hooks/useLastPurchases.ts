...
export function useFanActivity(accountIds: string[]): FanActivity {
  const queries = useQueries({
    queries: accountIds.map((aid) => ({
      ...
      // realtime chat_message event (price>0). So once we've snapshotted
      // the ledger once per session, we should never refetch from OF.
      // - `staleTime: Infinity` keeps the cache fresh forever
      // - `refetchOnMount: false` stops React Query from re-issuing the
      //    request when ChatList or FanDrawer remount mid-session
      // - Manual refresh is still available via `qc.invalidateQueries(["last-purchases"])`.
      staleTime: Infinity,
      gcTime: 24 * 60 * 60_000,
