...
export function MediaTile({
  ...
  if (failed) {
    return (
      <div
        className={cn(
          "aspect-video bg-bg/30 rounded-md grid place-items-center text-[10px] opacity-70",
          className,
        )}
      >
        failed
  ...
  if (!blobUrl) {
    return <div ref={placeholderRef} className={cn("bg-bg/30 rounded-md", className)} />;
  }
  return <img src={blobUrl} alt={alt} decoding="async" className={className} />;
}
